      ...
      slideRefs.current.forEach((slideRef, index) => {
        if (slideRef) {
          ...
          index === 3 &&
            console.log(
              slideLeftPos,
              Math.ceil(contentScrollPos + gap),
              index,
              "gap",
              gap,
            );
          ...
          if (
            slideLeftPos <=
            Math.ceil(contentScrollPos + gap /* + slideWidth/2 */)
          ) {
            currentIndex = index;
          }
        }
  ...
  return (
    <section className={styles.container}>
        ...
        {Array.isArray(children) ? (
          children.map((child, index) => (
            <div
              key={index}
              ref={(ref) => {
                ref && (slideRefs.current[index] = ref);
              }}
      ...
      <div className={styles.dots}>
        <Dots
          length={Array.isArray(children) ? children.length - 2 : 1}
